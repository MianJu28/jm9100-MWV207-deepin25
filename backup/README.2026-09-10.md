...
>
> **VA-API 直通已打通 ✅（2026-09-10）**：`mpv --hwdec=vaapi`（VA surface 经
> dmabuf 零拷贝交给 GL）此前画面全绿。根因两处叠加：①VA 驱动把解码 surface
> 池作为一整块连续显存申请，可见窗口（仅 255MB）被桌面打碎后整组回退到
> **CPU 不可见显存池**；②mpv 在 desktop GL 下走 JM9100 未真正实现的
> `glEGLImageTargetTexStorageEXT` 导入入口，纹理恒为全零。
> 修复：mpv 侧补丁改用 `glEGLImageTargetTexture2DOES`（`mpv_dmabuf_oes_image.patch`）
> \+ 内核 dmabuf 导出链修正（同驱动导入快捷路径等）。
> **使用方法与自检脚本见 FIXLOG.md「VA-API 直通使用方法」**。
