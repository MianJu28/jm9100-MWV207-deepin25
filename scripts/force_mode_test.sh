...
#
# 用法(SSH 会话, 必须 root):
#   sudo ./scripts/force_mode_test.sh          # 先跑 live 诊断(自动回退, 不影响后续)
#   sudo ./scripts/force_mode_test.sh boot     # live 后再跑, 准备重启验证
#   sudo reboot                        # 重启后观察屏幕 / SSH 检查
#   sudo ./scripts/force_mode_test.sh boot-undo && sudo reboot   # 结束后回滚
#
