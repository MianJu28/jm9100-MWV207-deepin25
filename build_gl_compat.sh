#!/usr/bin/env bash
#
# build_gl_compat.sh -- 编译/安装 JM9100 用户态 GL 兼容层（jm_gl_compat.c）
#
# 兼容层做两件事（详见 jm_gl_compat.c 头部注释与 README §3.6）：
...
#
# 结果：不必再给每个应用打补丁（如 mpv_dmabuf_oes_image.patch），系统原版
#       mpv / 其它走 storage 路径的客户端即可进入 VA-API dmabuf 零拷贝直通。
#
# 生效方式：LD_PRELOAD。**不替换、不覆盖任何厂商库**，因此可随时停用。
#
# 用法：
#   ./build_gl_compat.sh               # 只编译到 build-cli/libjm_gl_compat.so
#   ./build_gl_compat.sh install       # 编译 + 安装到 /usr/lib/aarch64-linux-gnu/
#   ./build_gl_compat.sh uninstall     # 卸载系统副本
#
# 安装后按需使用（任选其一）：
...
build() {
	[ -f "$SRC" ] || die "找不到 $SRC（请在 jm9100 仓库根目录运行）"
	mkdir -p "$OUT_DIR"
	say "编译 $SO_NAME ..."
